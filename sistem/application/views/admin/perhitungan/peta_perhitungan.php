...
<?php
$dataMap = array_values($_SESSION["array1"]);
$batas_awal = array_values($_SESSION["array2"]);
$batas_akhir = array_values($_SESSION["array3"]);
...
{    
    ...
    $valAhp[$i] = $dataMap[$i][2];
    $nama_kecamatan[$i] = $dataMap[$i][1];
...
echo "var batasawal = ['$batas_awal[0]','$batas_awal[1]','$batas_awal[2]'];";
echo "var batasakhir = ['$batas_akhir[0]','$batas_akhir[1]','$batas_akhir[2]'];";
...
function init() {
    ...
    for (i = 0; i < polys.length; i++) {
        ...
        if(tmpvalahp >= batasawal[0] && tmpvalahp <= batasakhir[0]) //cluster rawan rendah
        {
            if (tmp.length) {
...
//=========================================================================================================================
		if(tmpvalahp >= batasawal[1] && tmpvalahp <= batasakhir[1]) //cluster rawan sedang
        {
            if (tmp.length) {
            polys[i] = new google.maps.Polygon({
                paths : tmp,
            ...
            polys[i].setMap(map);
                    ...
                    google.maps.event.addListener(polys[i], 'click', function (event) {
                        ...
                        infoWindow.open(map);
                        var contentString = '<b>ID : ' +this.indexID +'<br> Value AHP : '+this.val + '<br> Kecamatan : ' +this.kec + '<br> Keterangan : Tingkat Rawan Banjir Sedang' ;
...
//===========================================================================================================================
        ...
        else if(tmpvalahp >= batasawal[2] && tmpvalahp <= batasakhir[2]) //cluster tinggi
        {
            if (tmp.length) {
            polys[i] = new google.maps.Polygon({
                paths : tmp,
